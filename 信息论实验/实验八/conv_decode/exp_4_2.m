...
for k = 1:length(EbN0db)
    for i = 1:N
        ...
        %——卷积码（3，1，3）编码过AWGN——
        C_conv2 = conv_encode31n(g0,g1,g2,input);%编码
        ...
        %——卷积码（2，1，3）编码过AWGN——
        C_conv = conv_encode21n(g0,g1,input);%编码
        Tx_conv2 = C_conv.*2-1;%编码后调制
...
legend('(3,1,3)卷积码编码值','未编码值','(2,1,3)卷积码编码值')
grid on
xlabel('信噪比dB')
ylabel('误码率')